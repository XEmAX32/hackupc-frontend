      ...
      <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', width: '100%'}}>
          {args.options.map((opt, i) => (
            <div key={i} className="question-option" style={{backgroundColor: ((i == args.correct || args.correct == "*") && selected) ? '#64FCD9' : '#FFF'}} onClick={() => (i == args.correct || args.correct == "*") ? success() : args.errouneousCallback()}>
              {opt}
            </div>
          ))}
        </div>
